...
function plugin(options) {
  ...
  function extendFunc(name, func) {
    extendFunctions[name] = function(path, density) {
      var normalizedPath = unquote(unescapeCss(path));
      return measure(normalizedPath, density).then(func);
    };
  }

  if (params.extend) {
    for (var funcName in params.extend) {
      extendFunc(funcName, params.extend[funcName]);
    }
  }
  ...
  return postcss()
    ...
    .use(functions({
      functions: Object.assign(extendFunctions, {
